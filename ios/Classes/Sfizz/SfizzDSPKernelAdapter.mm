...
- (bool)loadSfzFile:(NSString *)path {
    std::string cppPath ([path fileSystemRepresentation]);
    
    return _kernel.loadFile(cppPath);
}

- (bool)loadSfzString:(NSString *)sampleRoot sfzString:(NSString *) sfzString {
    std::string cppSampleRoot ([sampleRoot fileSystemRepresentation]);
    std::string cppSfzString ([sfzString UTF8String]);
    
    return _kernel.loadString(cppSampleRoot, cppSfzString);
}
